...
function UI.draw()
    ...
    if debug then
        ...
        debugText("player.pos - " .. tostring(math.floor(GameState.player.x.pos)) .. " / " .. tostring(math.floor(GameState.player.y.pos)))

        Fx.r.text(
            string.format(
                "x: %.1f\ny: %.1f\nz: %.1f\nvz: %.2f\ngrounded: %s",
                GameState.player.x.pos,
                GameState.player.y.pos,
                GameState.player.z.pos or 0,
                GameState.player.z.vel or 0,
                tostring(GameState.player.meta.move.grounded)
            ),
            Game.width-110,
